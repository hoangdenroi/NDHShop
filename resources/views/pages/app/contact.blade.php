...
@section('content')
    ...
    <div @class(['bg-white', 'dark:bg-slate-800', 'border', 'border-slate-100', 'dark:border-slate-700', 'rounded-2xl', 'shadow-sm', 'overflow-hidden'])>
        <div @class(['max-w-7xl', 'mx-auto', 'px-6', 'py-16', 'md:py-24'])>
            ...
            <p @class(['text-slate-600', 'dark:text-slate-400', 'text-xl', 'max-w-2xl'])>Chúng tôi luôn sẵn sàng hỗ trợ bạn. Hãy gửi tin nhắn và chúng tôi sẽ phản hồi trong vòng 24 giờ.</p>
        </div>
    </div>
                ...
                <form x-data="{
                    ...
                    <div @class(['grid', 'md:grid-cols-2', 'gap-6'])>
                        <label @class(['flex', 'flex-col', 'gap-2'])>
                            <span @class(['text-slate-700', 'dark:text-slate-300', 'text-sm', 'font-semibold'])>Họ và tên <span @class(['text-red-500'])>*</span></span>
                            <input x-model="form.name" required @class(['form-input', 'rounded-lg', 'border-slate-200', 'dark:border-slate-700', 'bg-background-light', 'dark:bg-slate-800', 'focus:border-primary', 'focus:ring-1', 'focus:ring-primary', 'h-12', 'px-4', 'text-base']) placeholder="Nguyễn Văn A" type="text"/>
                        </label>
                        <label @class(['flex', 'flex-col', 'gap-2'])>
                            <span @class(['text-slate-700', 'dark:text-slate-300', 'text-sm', 'font-semibold'])>Địa chỉ Email <span @class(['text-red-500'])>*</span></span>
                            <input x-model="form.email" required @class(['form-input', 'rounded-lg', 'border-slate-200', 'dark:border-slate-700', 'bg-background-light', 'dark:bg-slate-800', 'focus:border-primary', 'focus:ring-1', 'focus:ring-primary', 'h-12', 'px-4', 'text-base']) placeholder="email@example.com" type="email"/>
                        </label>
                    </div>
                    <label @class(['flex', 'flex-col', 'gap-2'])>
                        <span @class(['text-slate-700', 'dark:text-slate-300', 'text-sm', 'font-semibold'])>Chủ đề</span>
                        <input x-model="form.subject" @class(['form-input', 'rounded-lg', 'border-slate-200', 'dark:border-slate-700', 'bg-background-light', 'dark:bg-slate-800', 'focus:border-primary', 'focus:ring-1', 'focus:ring-primary', 'h-12', 'px-4', 'text-base']) placeholder="Chúng tôi có thể giúp gì cho bạn?" type="text"/>
                    </label>
                    <label @class(['flex', 'flex-col', 'gap-2'])>
                        <span @class(['text-slate-700', 'dark:text-slate-300', 'text-sm', 'font-semibold'])>Nội dung tin nhắn <span @class(['text-red-500'])>*</span></span>
                        <textarea x-model="form.message" required @class(['form-textarea', 'rounded-lg', 'border-slate-200', 'dark:border-slate-700', 'bg-background-light', 'dark:bg-slate-800', 'focus:border-primary', 'focus:ring-1', 'focus:ring-primary', 'p-4', 'text-base', 'resize-none']) placeholder="Hãy cho chúng tôi biết thêm về yêu cầu của bạn..." rows="6"></textarea>
                    </label>
                    ...
                    <div @class(['flex', 'gap-4'])>
                        ...
                        <div>
                            ...
                            <div @class(['flex', 'gap-4', 'mt-2'])>
                                ...
                                <a @class(['text-slate-500', 'hover:text-primary', 'transition-colors', 'flex', 'items-center', 'justify-center', 'p-2', 'rounded-full', 'hover:bg-slate-100', 'dark:hover:bg-slate-800']) href="#">
                                    <svg @class(['w-5', 'h-5', 'fill-current']) viewBox="0 0 448 512"><path d="M448,209.91a210.06,210.06,0,0,1-122.77-39.25V349.38A162.55,162.55,0,1,1,185,188.31V278.2a74.62,74.62,0,1,0,52.23,71.18V0l88,0a121.18,121.18,0,0,0,1.86,22.17h0A122.18,122.18,0,0,0,381,102.39a121.43,121.43,0,0,0,67,20.14Z"/></svg>
                                </a>
                                <a @class(['text-slate-500', 'hover:text-primary', 'transition-colors', 'flex', 'items-center', 'justify-center', 'p-2', 'rounded-full', 'hover:bg-slate-100', 'dark:hover:bg-slate-800']) href="#">
