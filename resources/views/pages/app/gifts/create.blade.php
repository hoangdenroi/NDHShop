...
@section('content')
    <div class="container mx-auto px-4 py-8 max-w-2xl">
        ...
        <div class="mb-8">
            ...
            <div class="flex items-center gap-4">
                <h1 class="text-2xl md:text-3xl font-black text-slate-900 dark:text-white">
                    Tạo thiệp: <span class="text-primary">{{ $template->name }}</span>
                </h1>
        ...
        {{-- Form Nhập Liệu (Tự động sinh từ JSON Schema) --}}
        <div class="bg-white dark:bg-surface-dark border border-slate-200 dark:border-border-dark rounded-2xl shadow-sm overflow-hidden">
            <div class="bg-primary/5 border-b border-primary/10 p-5">
                ...
                <p class="text-xs text-slate-500 mt-1">Điền thông tin bên dưới để cá nhân hóa món quà của bạn.</p>
            </div>
            ...
            <form method="POST" action="{{ route('app.gifts.store', $template->slug) }}" class="p-5 grid grid-cols-2 gap-5">
                ...
                @foreach($template->getSchemaFields() as $field)
                    @php
                        ...
                        $oldValue = old("data.{$field['key']}", $field['default'] ?? '');
                    ...
                    <div class="{{ $width === 'half' ? 'col-span-2 sm:col-span-1' : 'col-span-2' }}">
                        ...
                        @if($type === 'image')
                            ...
                            <div x-data="imageUploader('{{ $field['key'] }}', {{ $limit }}, '{{ $oldValue }}')" class="space-y-3">
                                {{-- Hidden input chứa URLs phân cách \n --}}
                ...
                <div class="col-span-2 mt-4 pt-5 border-t border-slate-100 dark:border-border-dark flex flex-col gap-3">
                    @if($template->is_premium && $template->price > 0)
                        {{-- Template premium → thanh toán luôn --}}
                        <button type="submit" class="w-full flex items-center justify-center gap-2 py-3 bg-gradient-to-r from-primary to-purple-600 hover:from-primary/90 hover:to-purple-600/90 text-white font-bold rounded-xl shadow-lg shadow-primary/30 transition-all transform hover:-translate-y-0.5">
                            ...
                            Thanh toán {{ number_format($template->price, 0, ',', '.') }}đ
                        </button>
                        <p class="text-center w-full text-xs text-slate-500 font-medium flex items-center justify-center gap-1">
                            <span class="material-symbols-outlined text-[14px]">info</span>
                            Mẫu Premium — Số tiền sẽ được trừ từ số dư tài khoản
                        </p>
                    @else
                        {{-- Template thường → chọn gói trước --}}
                        <button type="submit" class="w-full flex items-center justify-center gap-2 py-3 bg-gradient-to-r from-primary to-purple-600 hover:from-primary/90 hover:to-purple-600/90 text-white font-bold rounded-xl shadow-lg shadow-primary/30 transition-all transform hover:-translate-y-0.5">
                        ...
                        <p class="text-center w-full text-xs text-slate-500 font-medium flex items-center justify-center gap-1">
                            <span class="material-symbols-outlined text-[14px]">info</span>
                            Bước tiếp theo: Chọn gói Basic (miễn phí) hoặc Premium (49.000đ)
                        </p>
                    @endif
    ...
    <script>
        ...
        function imageUploader(fieldKey, maxImages, initialValue) {
            return {
                ...
                init() {
                    // Parse old value (URLs phân cách \n)
                ...
                initSortable() {
                    ...
                    // FIX: Khi kéo trên mobile, SortableJS tạo clone (fallback)
                    // để theo ngón tay. Clone nằm ngoài scope x-for của Alpine
                    // → biến img, index không tồn tại → lỗi.
                    // Giải pháp: Chặn tại appendChild/insertBefore,
                    // gắn x-ignore trước khi Alpine kịp xử lý clone.
                    ...
                    this._sortable = Sortable.create(grid, {
                        animation: 200,
                        handle: '.drag-handle',           // Chỉ kéo qua icon handle
                        ghostClass: 'sortable-ghost',     // Class cho vị trí gốc (mờ đi)
                        chosenClass: 'sortable-chosen',   // Class khi chọn
                        dragClass: 'sortable-drag',       // Class khi đang kéo
                        fallbackClass: 'sortable-fallback', // Class cho clone theo ngón tay
                        filter: '[data-sortable-ignore]', // Bỏ qua nút "Thêm ảnh"
                        preventOnFilter: false,
                        fallbackOnBody: false,             // Clone nằm trong grid (để override bắt được)
                        delay: 150,                        // Delay nhẹ trên mobile tránh xung đột scroll
                        delayOnTouchOnly: true,            // Chỉ delay trên touch
                        touchStartThreshold: 5,            // Ngưỡng pixel trước khi bắt đầu kéo
                        onEnd(evt) {
                            if (evt.oldIndex === evt.newIndex) return;

                            // Đọc thứ tự mới từ DOM.
                            // QUAN TRỌNG: Lúc onEnd, clone fallback + ghost
                            // vẫn trong DOM → phải loại trừ để tránh trùng ID.
                            const seen = new Set();
                            ...
                            // Rebuild mảng images theo thứ tự DOM mới
                            ...
                            if (reordered.length === self.images.length) {
                                // Xóa sạch để Alpine dỡ hết DOM cũ (tránh xung đột SortableJS)
                                self.images = [];
                                // Tick sau: gán mảng mới → Alpine render lại từ đầu, đúng thứ tự
                                self.$nextTick(() => {
                ...
                replaceImage(index) {
                    // Tạo input file tạm để chọn ảnh thay thế
