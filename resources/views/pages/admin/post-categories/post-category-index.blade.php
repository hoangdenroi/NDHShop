<x-admin-layout title="NDHShop - Admin - Danh mục Bài viết">
    <div class="flex flex-col gap-6">
        ...
        <div
            ...
            <div class="flex items-center gap-2 shrink-0">
                <button x-data x-on:click="$dispatch('open-modal', 'create-post-category')"
                    ...
                    <thead>
                        <tr
                            ...
                            <th
                                ...
                                Tên danh mục</th>
                            <th
                                class="p-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-center">
                                Số bài viết</th>
                            <th
                                class="p-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                                Trạng thái</th>
                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-background-dark/30 transition-colors group {{ $category->is_deleted ? 'opacity-60' : '' }}">
                                ...
                                <td class="p-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <button x-data
                                            x-on:click="$dispatch('open-edit-post-category', {{ json_encode(['id' => $category->id, 'name' => $category->name, 'slug' => $category->slug, 'description' => $category->description, 'is_active' => $category->is_active]) }})"
                                        ...
                                        @if(!$category->is_deleted)
                                        <button x-data
                                            x-on:click="$dispatch('open-delete-post-category', {{ json_encode(['id' => $category->id, 'name' => $category->name, 'posts_count' => $category->posts_count ?? 0]) }})"
                            ...
                            <tr>
                                <td colspan="9" class="p-8 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <p class="text-slate-500 text-sm">Không tìm thấy danh mục bài viết nào.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
    ...
    <script>
        ...
        function applyFilters() {
            ...
            window.location.href = '{{ route("admin.post-categories.index") }}?' + params.toString();
        }
    </script>

    {{-- Modal CRUD --}}
    <x-admin.post-category-crud.modal-create />
    <x-admin.post-category-crud.modal-edit />
    <x-admin.post-category-crud.modal-delete />
</x-admin-layout>
