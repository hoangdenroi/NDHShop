    ...
    <form method="POST" action="{{ route('admin.gift-templates.store') }}" class="p-6" x-data="{
        ...
        <div x-show="activeTab === 'schema'" class="flex flex-col gap-4">
            <div>
                ...
                <p class="text-xs text-slate-400 mb-1">Định nghĩa các trường nhập liệu cho user. Ví dụ bên dưới.</p>
                <textarea name="schema" rows="14"
                    class="w-full bg-slate-100 dark:bg-background-dark border border-slate-200 dark:border-border-dark text-slate-700 dark:text-slate-300 text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary font-mono"
                    placeholder="Nhập JSON Schema tại đây">{
  "fields": [
    { "key": "TITLE", "type": "text", "label": "Tiêu đề", "required": true, "default": "Câu chúc của bạn" },
    { "key": "MESSAGE", "type": "textarea", "label": "Lời chúc", "required": true, "default": "Gửi lời chúc tốt đẹp đến bạn", "limit": 5 },
    { "key": "IMAGE", "type": "image", "label": "Ảnh", "required": false, "limit": 2 }
  ]
}</textarea>
            </div>
        </div>
