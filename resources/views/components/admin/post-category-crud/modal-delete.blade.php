...
<x-ui.modal name="delete-post-category" maxWidth="md">
    <div x-data="{
            category: { id: null, name: '', posts_count: 0 }
        }"
        @open-delete-post-category.window="category = $event.detail; $dispatch('open-modal', 'delete-post-category')">
        <form method="POST" x-bind:action="`{{ url('admin/post-categories') }}/${category?.id}`" class="p-6">
            @csrf
            @method('DELETE')
            ...
            <div class="mb-6">
                ...
                <div class="mt-4 p-3 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-lg flex gap-3">
                    ...
                    <p class="text-xs text-amber-700 dark:text-amber-400 font-medium">
                        Khi xóa, danh mục này sẽ bị ẩn đi. Chỉ có thể xóa khi danh mục không chứa bất kỳ bài viết nào (<span x-text="category?.posts_count || 0"></span> bài viết).
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-3">
                <button type="button" x-on:click="$dispatch('close-modal', 'delete-post-category')"
                ...
                <button type="submit"
                    ...
                    x-bind:disabled="category?.posts_count > 0">
                    Chắc chắn xóa
                </button>
            </div>
        </form>
