<x-ui.modal name="edit-post-category" maxWidth="md">
    <div x-data="{
            category: { name: '', slug: '', description: '', is_active: false }
        }"
        @open-edit-post-category.window="category = $event.detail; $dispatch('open-modal', 'edit-post-category')">
        <form method="POST" x-bind:action="`{{ url('admin/post-categories') }}/${category?.id}`" class="p-6">
            ...
            <div
                ...
                <button type="button" x-on:click="$dispatch('close-modal', 'edit-post-category')"
