...
class GiftRenderService
{
    ...
    public function render(GiftPage $giftPage): string
    {
        ...
        //    Hỗ trợ cả {{KEY}}, {{ KEY }}, {{  KEY  }} (có/không space)
        ...
        $htmlCode = preg_replace('/\{\{\s*[A-Z_0-9]+\s*\}\}/', '', $htmlCode);

        return $htmlCode;
    }

    /**
     * Render trực tiếp từ template + data (dùng cho demo, không cần GiftPage).
     */
    public function renderFromTemplate(GiftTemplate $template, array $data): string
    {
        // 1. Giải mã Base64
        $htmlCode = $template->decoded_html;
        $cssCode = $template->decoded_css;
    ...
    /**
     * Inject watermark cho gói basic hoặc badge DEMO.
     */
...
HTML;
    ...
    public function injectMetaTags(string $html, GiftPage $giftPage): string
    {
        ...
        $url = url("/gifts/{$giftPage->share_code}");
